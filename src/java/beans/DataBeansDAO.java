...
public class DataBeansDAO {
    ...
    public ArrayList<DataBeansDTO> getRankingData() throws SQLException{
        ...
        try{
            ...
            while(rs.next()){
                DataBeansDTO resultDb = new DataBeansDTO();
                resultDb.setName(rs.getString(1));
                resultDb.setTotalPoints(rs.getInt(2));
    ...
    */
    ...
    public void userDataDelete(DataBeansDTO ud) throws SQLException{
        Connection con = null;
        PreparedStatement st = null;
        try{
            con = DBManager.getConnection();
            ...
            st.setInt(1, ud.getUserID());
            st.executeUpdate();
            st =  con.prepareStatement("delete from review where userID = ?");
            st.setInt(1, ud.getUserID());
            st.executeUpdate();
            System.out.println("delete completed");
        }catch(SQLException e){
    ...
    *同名アプリを追加するとややこしくなるため
    */
    public void deletedUserDataRestoration(DataBeansDTO ud) throws SQLException{
        Connection con = null;
        PreparedStatement st = null;
        try{
            con = DBManager.getConnection();
            st =  con.prepareStatement("update user_t set deleteFlg = 0 where userID=?");
            st.setInt(1, ud.getUserID());
            st.executeUpdate();
            System.out.println("update completed");
        }catch(SQLException e){
    ...
    public ArrayList<DataBeansDTO> getDeletedUserData() throws SQLException{
        ...
        try{
            con = DBManager.getConnection();
            st =  con.prepareStatement("select userID,userName from user_t where deleteFlg = 1");
            ResultSet rs = st.executeQuery();
            ArrayList<DataBeansDTO> userInfo = new ArrayList();
            
            while(rs.next()){
                DataBeansDTO resultDb = new DataBeansDTO();
                resultDb.setUserID(rs.getInt(1));
                resultDb.setName(rs.getString(2));
                userInfo.add(resultDb);                
            }
            System.out.println("select completed");
            return userInfo;
